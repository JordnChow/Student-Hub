function Button() {
	localStorage.setItem("MainColour", document.getElementById("maincolour").value)
	prompt(localStorage.getItem("MainColour"))
}
...
function ChangeColour() {
	...
	let newclass = [colour + "text", colour + "contrast", colour + "dark", colour + "nav", colour + "scrollbar", colour + "main"]
	let changeclass = ["changetext", "changecontrast", "changedark", "changenav", "changescrollbar", "changemain"]
	let oldclass = [oldcolour + "text", oldcolour + "contrast", oldcolour + "dark", oldcolour + "nav", oldcolour + "scrollbar", oldcolour + "main"]
	if (colour == "standard") {
		document.body.style.backgroundColor = "#e6eff1"
	} else if (colour == "cool") {
		document.body.style.backgroundColor = "#98fb98"
	} else if (colour == "warm") {
		document.body.style.backgroundColor = "#faf0f0"
	} else if (colour == "neutral") {
		document.body.style.backgroundColor = "#e3e3e3ff"
	}
	let x = 0
...
function QuizQuestion() {
	...
	if (QuestionType == "4-Answer") {
		...
		UpdateScores(score, subject, CurrentT)
		if (score == 10) {
...
function UpdateScores(Score, Subject, Topic) {
	...
	var topicscore = (parseInt(localStorage.getItem(Subject + Topic + "Score"))/10)
	if (!topicscore) {
		var topicscore = Score
	} else {
		topicscore += Score
	}
	var subjectscore = (parseInt(localStorage.getItem(Subject + "Score"))/10)
	if (!subjectscore) {
		var subjectscore = Score
	} else {
		subjectscore += Score
	}
	var generalscore = (parseInt(localStorage.getItem("GeneralScore"))/10)
	if (!generalscore) {
		var generalscore = Score
	} else {
		generalscore += Score
	}
	var mainsubjectscore = (parseInt(localStorage.getItem(MainSubject + "Score"))/10)
	if (!mainsubjectscore) {
	...
	var topictries = parseInt(localStorage.getItem(Subject + Topic + "Tries"))
	if (!topictries) {
		var topictries = 1
	} else {
		topictries += 1
	}
	var subjecttries = parseInt(localStorage.getItem(Subject + "Tries"))
	if (!subjecttries) {
		var subjecttries = 1
	} else {
		subjecttries += 1
	}
	var generaltries = parseInt(localStorage.getItem("GeneralTries"))
	if (!generaltries) {
		var generaltries = 1
	} else {
		generaltries += 1
	}
	var mainsubjecttries = parseInt(localStorage.getItem(MainSubject + "Tries")) + 1 
	if (!mainsubjecttries) {
	...
	var topicpercent = parseInt(topicscore / topictries) * 10
	var subjectpercent = parseInt(subjectscore / subjecttries) * 10
	var generalpercent = parseInt(generalscore / generaltries) * 10
	var mainsubjectpercent = parseInt(mainsubjectscore / mainsubjecttries) * 10
